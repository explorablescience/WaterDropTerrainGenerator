    ...
    V: SnarlViewer<T>,
{
    ...
    for out_pin in outputs {
        ...
        outputs_ui.scope_builder(builder, |pin_ui| {
            ...
            //
            // This row uses a right-to-left layout, so `next_widget_position()` returns the
            // *right* edge of the space still available for a zero-size widget. The reserved
            // rect must grow leftward from there; growing it rightward (as
            // `Rect::from_min_size` would, treating that point as the left/top corner)
            // overflows past the row's bounds and permanently widens this ui's `max_rect`,
            // which then compounds on every subsequent row.
