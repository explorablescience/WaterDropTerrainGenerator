...
src/
  ...
  nodes/                 # Concrete Node implementations (generation, modification, export)
...
`NodeGraph` owns a `Topology` (pure structure: nodes/edges, stable `GraphNodeId`s via tombstoning), an `EvalCache` (`Mutex<HashMap<(GraphNodeId, EvalScope), NodeState>>` — `EvalScope` is `Chunk(ChunkCoord)` or `Global(native_resolution)`, so the same node can cache a different tile per chunk plus one entry per `Global` resolution it's ever been evaluated at), a shared `TilePool`, and the `ChunkGrid`. `mark_dirty` propagates downstream through the topology and stops at `Baked` nodes — it never reaches a node's *ancestors*, which is exactly why `EvalScope::Global` carries the resolution rather than relying on invalidation: a `Local` node promoted into a `Global` pass (see below), or a `Global` node re-evaluated after its own `native_resolution` param changes, would otherwise silently reuse a differently-sized cached tile from a stale resolution and panic.
...
`core/graph/eval.rs` is the evaluation engine: topological recursion with cycle detection, `required_internal_tile_size` sums `Local` ancestors' kernel padding to size tile margins, `Global` ancestors stop fan-out (their kernel padding is self-contained). Any `Global` ancestor's output is bilinearly resampled (`resample_tile`) into whatever frame the consuming node needs — a `Local` chunk, or another `Global` pass at a different resolution — so a `Global` node's output can be wired directly into anything of matching socket `dtype`, in either direction, with no manual placement node and no locality check at connect time (`Topology::connect` only validates `dtype`). A `Local` ancestor pulled into a `Global` node's own pass, conversely, is evaluated directly in that pass's frame rather than resampled (it was never computed anywhere else) — this is the graph's "bake a local generator once at global scope" pattern, e.g. a `Perlin` noise node feeding a whole-domain erosion pass. `process_chunk_shared` is what background chunk jobs call; `process_sync` drives the (non-parallelizable) `Global`-locality preview path directly.

### Chunking (`core/tiling`)

`ChunkGrid { chunks_x, chunks_y, tile_size, world_scale }` + `ChunkCoord(i32, i32)` is the terrain's chunk partitioning, configured from the UI's Terrain Settings panel. `TileContext` is the position-aware frame passed to `Node::process`: world origin/step/extent for a given chunk, or — for a `Global` node (`TileContext::for_global`) — the same real world extent the whole chunk grid covers (`ChunkGrid::world_extent()`), evaluated as a single tile at the node's own `native_resolution` instead of split across chunks. Sharing the terrain's real extent, rather than some self-contained canonical space, is what lets a `Local` node promoted into a `Global` pass mean the same thing it would evaluated normally per chunk (a noise generator's frequency reads the same either way). `TilePool`/`TileHandle` is a free-list allocator for the `Vec<f32>` tile buffers, returned to the pool on `Drop`.
