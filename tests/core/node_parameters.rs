use waterdrop_terrain_editor::core::node_parameters::{NParamConstraints, NParamValue};
...
fn string_max_length_accepts_short_strings() {
    ...
    assert!(constraint.validate(&NParamValue::String("abcde".to_string())).is_ok());
}
...
fn enum_one_of_accepts_listed_options() {
    let constraint = NParamConstraints::EnumOneOf { options: vec!["A", "B", "C"] };
    assert!(constraint.validate(&NParamValue::Enum("B".to_string())).is_ok());
}

#[test]
fn enum_one_of_rejects_unlisted_options() {
    let constraint = NParamConstraints::EnumOneOf { options: vec!["A", "B", "C"] };
    assert!(constraint.validate(&NParamValue::Enum("D".to_string())).is_err());
}
...
fn mismatched_constraint_and_value_types_fail_validation() {
    ...
    assert!(constraint.validate(&NParamValue::String("5".to_string())).is_err());
}
...
fn param_value_equality_ignores_the_action_success_flag() {
    ...
    assert_eq!(
        NParamValue::Action { show_success_message: true },
        NParamValue::Action { show_success_message: true }
    );
    assert_ne!(
        NParamValue::Action { show_success_message: true },
        NParamValue::Action { show_success_message: false }
    );
}
